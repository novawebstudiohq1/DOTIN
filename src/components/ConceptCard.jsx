...
function ConceptCard({
  ...
  return (
    <>
      <motion.article
        className="group relative isolate overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900 shadow-2xl shadow-black/10 transition-colors duration-300 hover:border-blue-500/50"
      ...
      >
        ...
        >
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-b from-zinc-950/75 via-transparent to-zinc-950/95" />
          <div className="absolute inset-x-0 top-0 flex items-start justify-between gap-4 p-4 sm:p-5">
            <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-zinc-950/70 px-3 py-1.5 font-mono text-[10px] font-medium uppercase tracking-[0.14em] text-white backdrop-blur-md">
          ...
          <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-4 p-4 sm:p-5">
            <div>
              <span className="mb-2 flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.16em] text-zinc-400">
            ...
            <span className="shrink-0 rounded-lg border border-white/15 bg-zinc-950/75 px-3 py-2 font-mono text-[9px] uppercase tracking-[0.12em] text-zinc-200 backdrop-blur-md">
              {price}
            </span>
          </div>
        </button>
      ...
      <AnimatePresence>
        {isModalOpen && (
          ...
          >
            <motion.section
              className="relative grid max-h-[calc(100dvh-2rem)] min-h-0 w-full max-w-6xl overflow-y-auto rounded-2xl border border-zinc-700 bg-zinc-900 shadow-2xl shadow-black/50 lg:grid-cols-[minmax(0,1.5fr)_minmax(280px,.65fr)]"
              role="dialog"
              aria-modal="true"
            ...
            >
              <button
                type="button"
                onClick={() => setIsModalOpen(false)}
                className="absolute right-4 top-4 z-10 grid h-9 w-9 place-items-center rounded-full border border-white/15 bg-zinc-950/75 text-zinc-300 backdrop-blur-md transition hover:border-blue-500/60 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
                aria-label="Close concept details"
              >
                <X size={17} />
              </button>
              <div className="relative min-h-[280px] bg-black lg:min-h-full">
                {isFrameLoading && <div className="absolute inset-0 z-10 grid place-items-center bg-zinc-950 text-[10px] uppercase tracking-[0.16em] text-zinc-400">Loading preview...</div>}
                <iframe
              ...
              <aside className="flex flex-col p-6 sm:p-8">
                <div>
                  ...
                  <h2 id={`${project.id ?? projectTitle}-title`} className="mt-5 text-3xl font-bold uppercase leading-none tracking-[-0.06em] text-white sm:text-4xl">
                    {projectTitle}
                  </h2>
                  <p className="mt-7 text-sm leading-7 text-zinc-400">{projectObjective}</p>
                </div>
                <div className="mt-8 border-t border-zinc-800 pt-6">
                  <h3 className="font-mono text-[10px] uppercase tracking-[0.16em] text-zinc-500">Deliverables</h3>
                  <ul className="mt-4 space-y-3">
                    {projectDeliverables.map((deliverable) => (
                      <li className="flex items-start gap-3 text-sm text-zinc-300" key={deliverable}>
