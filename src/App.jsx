...
const faqs = [
  ...
  { question: 'Can you develop a concept from scratch?', answer: 'Absolutely. Concept development is at the heart of DOT IN MEDIA. We can start from a product, a loose brief, or a feeling you want the audience to have.' },
...
const THEME_COLORS = { light: '#EDE6D8', dark: '#0d0d0c' }
    ...
    <div className="min-h-screen overflow-x-clip bg-bg text-fg">
      <motion.header initial={prefersReducedMotion ? false : { y: -18, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.6, ease: revealEase }} className="fixed inset-x-0 top-0 z-40 border-b border-line/80 bg-bg/85 backdrop-blur-xl">
        <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-5 lg:px-8">
          <a href="#top" className="flex items-center gap-2.5" aria-label="DOT IN MEDIA — home"><img src="/brand-mark.png" alt="" width="256" height="237" className="h-9 w-auto shrink-0" /><img src="/brand-wordmark.png" alt="DOT IN MEDIA" width="512" height="158" className="hidden h-[30px] w-auto sm:block" /></a>
          <nav className="hidden items-center gap-9 text-[11px] font-medium uppercase tracking-[0.18em] text-muted2 md:flex">
            {['Work', 'Process', 'FAQ'].map((link) => <a className="transition hover:text-fg" href={`#${link.toLowerCase()}`} key={link}>{link}</a>)}
          </nav>
      ...
      <main id="top" className="pt-[76px]">
        <motion.section initial={prefersReducedMotion ? false : 'hidden'} animate="visible" variants={revealVariants} transition={{ duration: 0.9, ease: revealEase }} className="relative mx-auto grid min-h-[720px] max-w-7xl items-center gap-12 px-5 py-20 lg:grid-cols-[.9fr_1.1fr] lg:px-8 lg:py-28">
          ...
          <Reveal className="relative z-10">
            <p className="mb-7 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.2em] text-muted"><span className="h-1.5 w-1.5 rounded-full bg-blue-500" /> Independent visual studio / Mumbai + worldwide</p>
        ...
        <motion.section initial={prefersReducedMotion ? false : 'hidden'} whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={revealVariants} transition={{ duration: 0.8, ease: revealEase }} className="border-t border-line bg-surface/35 dark:bg-black" id="faq"><div className="mx-auto max-w-5xl px-5 pt-14 pb-2 lg:px-8 lg:pt-20 lg:pb-3"><Reveal className="text-center"><p className="font-mono text-[10px] uppercase tracking-[0.2em] text-blue-500">No loose ends / 005</p><h2 className="mt-4 text-4xl font-bold tracking-[-0.07em] sm:text-6xl">Questions,<br /><span className="text-muted">answered.</span></h2></Reveal><div className="mx-auto mt-14 max-w-3xl border-t border-line2">{faqs.map((faq, index) => <motion.div initial={prefersReducedMotion ? false : { opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.1 }} transition={{ duration: 0.5, delay: prefersReducedMotion ? 0 : index * 0.05, ease: revealEase }} className="border-b border-line" key={faq.question}><button className="flex w-full items-center justify-between gap-6 py-6 text-left text-base font-medium" onClick={() => setOpenFaq(openFaq === index ? -1 : index)} aria-expanded={openFaq === index}><span><span className="mr-5 font-mono text-[10px] text-blue-500">0{index + 1}</span>{faq.question}</span>{openFaq === index ? <X size={17} className="shrink-0 text-blue-500" /> : <Plus size={17} className="shrink-0 text-muted" />}</button><div className={`grid transition-[grid-template-rows] duration-300 ${openFaq === index ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}><p className="min-h-0 overflow-hidden pl-10 pr-8 text-sm leading-7 text-muted">{faq.answer}</p></div></motion.div>)}</div></div></motion.section>
      </main>

      <motion.footer initial={prefersReducedMotion ? false : 'hidden'} whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={revealVariants} transition={{ duration: 0.8, ease: revealEase }} className="overflow-hidden">
        <div className="relative w-full overflow-hidden border-b border-line">
          {/* Theme-conditional footer images — both stay in the DOM for instant switching */}
          <img src="/light footer image.png" alt="Stories That Sell — DOT IN MEDIA film production studio" width="1983" height="793" className={`block h-auto w-full max-md:h-[520px] max-md:object-cover max-md:object-[40%_center] transition-opacity duration-500 ${theme === 'light' ? 'opacity-100' : 'opacity-0 absolute inset-0'}`} />
          <img src="/dark footer image.png" alt="Stories That Sell — DOT IN MEDIA film production studio" width="1983" height="793" className={`block h-auto w-full max-md:h-[520px] max-md:object-cover max-md:object-[40%_center] transition-opacity duration-500 ${theme === 'dark' ? 'opacity-100' : 'opacity-0 absolute inset-0'}`} />

          {/* CTA + footer content overlaid on the bottom portion of the image */}
          <div className="absolute inset-0 flex flex-col justify-end">
            <div className="relative z-10 px-5 pb-7 pt-16 lg:px-8">
              {/* CTA headline — left-aligned, above the footer bar */}
              <div className="mx-auto max-w-7xl mb-10 md:mb-14">
                <p className="font-mono text-[11px] uppercase tracking-[0.2em] font-medium !text-white text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.85)]" style={{ color: '#ffffff' }}>Have a story in mind?</p>
                <h2 className="mt-3 text-3xl font-bold tracking-[-0.07em] !text-white text-white drop-shadow-[0_3px_8px_rgba(0,0,0,0.9)] sm:text-5xl md:text-6xl" style={{ color: '#ffffff' }}>Stories That Sell</h2>
                <button onClick={() => setIsContactOpen(true)} className="mt-6 inline-flex items-center gap-5 bg-blue-600 text-white shadow-lg shadow-black/40 px-5 py-3.5 text-[10px] font-semibold uppercase tracking-[0.17em] transition hover:bg-blue-500">Get Quote <ArrowUpRight size={16} /></button>
              </div>

              {/* Footer bar */}
              <div className="mx-auto flex max-w-7xl flex-col justify-between gap-7 border-t border-white/20 pt-7 text-xs text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.85)] md:flex-row md:items-center">
                <a href="#top" className="shrink-0 drop-shadow-[0_2px_6px_rgba(0,0,0,0.5)]" aria-label="DOT IN MEDIA — home"><img src="/brand-lockup.png" alt="DOT IN MEDIA" width="480" height="406" className="h-[68px] w-auto" /></a>
                <div className="flex flex-wrap gap-5 uppercase tracking-[0.12em] font-medium"><a className="flex items-center gap-2 text-white/90 hover:text-white transition" href="tel:+919876543210"><Phone size={14} />+91 98765 43210</a><a className="flex items-center gap-2 text-white/90 hover:text-white transition" href="https://wa.me/919876543210" target="_blank" rel="noreferrer"><MessageCircle size={14} />WhatsApp</a><a className="flex items-center gap-2 text-white/90 hover:text-white transition" href="https://www.instagram.com/_dotin_media__/" target="_blank" rel="noreferrer"><Camera size={14} />Instagram</a></div>
                <span className="font-mono text-[10px] uppercase tracking-wider text-white/90">© {new Date().getFullYear()} DOT IN MEDIA</span>
              </div>
            </div>
          </div>
        </div>
